...
import_data <- function (file_url, name, file_downloaded_type) {
  local_path <- paste0("data/", basename(file_url))
  ...
  if(parsed_url$scheme == "ftp") {
    ...
    warn_for_status(
      httr::GET(
        url = file_url,
        write_disk(path.expand(local_path), overwrite=TRUE),
        progress("down")
      ),
      paste("dowload", file_name)
    )
  }
  
  # Unzip
  cat(paste("\n# Unzip start:", file_name, "\n"))
  if(tools::file_ext(file_url) == 'zip') utils::unzip(local_path , exdir = name)
}
